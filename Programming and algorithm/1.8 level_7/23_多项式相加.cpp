// 多项式相加
// 我们经常遇到两多项式相加的情况，在这里，
// 我们就需要用程序来模拟实现把两个多项式相加到一起。
// 首先，我们会有两个多项式，每个多项式是独立的一行，
// 每个多项式由系数、幂数这样的多个整数对来表示。
...
// 内存限制：65536
// 输入
// 输入包括多行。 第一行整数n,表示有多少组的多项式需要求和。(1 < n < 100) 
// 下面为2n行整数，每一行都是一个多项式的表达式。表示n组需要相加的多项式。 
// 每行长度小于300。
// 输出
// 输出包括n行，每行为1组多项式相加的结果。 在每一行的输出结果中，
// 多项式的每一项用“[x y]”形式的字符串表示，x是该项的系数、y 是该项的幂数。
...
// 解题思路：
// 对于这道题，我们可以使用一个数组来存储每一项的系数，然后使用另一个数组来存储每一项的幂数。
// 最后遍历两个数组，将系数相加，如果系数不为零，则输出结果。


#include<bits/stdc++.h>
using namespace std;
const int N=1005; // 定义数组长度
int n,m,k; // 定义整型变量n，m，k
int a[N],b[N]; // 定义数组a和b
...
int main()
{
    ...
    while(n--)
    {
        ...
        int cnt=0; // 定义整型变量cnt，表示数组b中存储的系数个数
        for(int i=1000;i>=0;i--)
        {
            if(a[i]!=0)
            {
                b[cnt++]=a[i]; // 如果a[i]不等于0，就将a[i]存储到数组b中
            }
        }
        cout<<cnt<<" "; // 输出数组b中存储的系数个数
        for(int i=0;i<cnt;i++)
        {
            cout<<b[i]<<" "; // 输出数组b中的系数
        }
