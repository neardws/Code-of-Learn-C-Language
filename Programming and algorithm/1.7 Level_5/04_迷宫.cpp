...
// 每组测试数据的第1行是一个正整数n (1 <= n <= 100)，
// 表示迷宫的规模是n * n的。
// 接下来是一个n * n的矩阵，矩阵中的元素为.或者#。
// 再接下来一行是4个整数ha, la, hb, lb，描述A处在第ha行, 第la列，
// B处在第hb行, 第lb列。
...
bool dfs(int x, int y) {  // x， y 表示当前的位置
    if (x == hb + 1 && y == lb + 1)  // 已到达b 点
        return true;
    int xx, yy;  // 新的位置
    ...
    while (k--) {
        ...
        for (int i = 0; i <= n + 1; i++) {
            for (int j = 0; j <= n + 1; j++) {//这里是将字符转化为数字了，也可以不转化
                if (i == 0 || j == 0 || i == n + 1 || j == n + 1)   // 地图四周的点
                    map[i][j] = 1;     // 值为1 表示不能通行
                else {
                    cin >> temp;        
                    map[i][j] = temp == '#' ? 1 : 0;    // # 等于1 表示 不能通行
                }
            }
        }
        cin >> ha >> la >> hb >> lb;       // 输入a，b 点的行列
        if (map[ha + 1][la + 1] == 1 || map[hb + 1][lb + 1] == 1) {
            // 如果 点 a 和点 b 本身是无法到达的
            cout << "NO" << endl;
            continue;
        }
