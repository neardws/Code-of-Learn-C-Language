...
// 描述
// 给出4个小于10正整数，你可以使用加减乘除4种运算以及括号把这4个数连接起来得到一个表达式。
...
#include<bits/stdc++.h>
using namespace std;
bool Op(double a[], int n){    // 传入数组，n表示运算数个数
    double Next[5];     //用来保存下一次运算数据
    if(n==1&&abs(a[0]-24)<0.00001)
                //因为是实数，所以不能直接进行判断
        return true;
    if(n==1)    // n=1 且 结果不等于24
        return false;
    for(int i=0; i<n; i++){     // i 表示 参与运算数1
        for(int j=i+1; j<n; j++){   // j 表示 参与运算数2
            int t=1;
            for(int k=0; k<n; k++){
                if(k!=i && k!=j)
