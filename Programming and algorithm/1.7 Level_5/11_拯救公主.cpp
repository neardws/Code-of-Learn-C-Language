...
// 你为阿福提供了一张大魔王根据地的地图，上面标记了阿福和公主所在的位置，以及一些不能够踏入的禁区。
// 你还贴心地为阿福制造了一些传送门，通过一个传送门可以瞬间转移到任意一个传送门，当然阿福也可以选择不通过传送门瞬移。
// 传送门的位置也被标记在了地图上。此外, 你还查探到公主所在的地方被设下了结界, 需要集齐K种宝石才能打开。当然,你在地图上也标记出了不同宝石所在的位置。
...
// 每一组数据的第一行包含了三个用空格分开的正整数R、C（2 <= R, C <= 200）和K，表示地图是一个R×C的矩阵，
...
//     我们需要定义一个二维int数组（door_coord）——第一维表示传送门的个数，第二维表示传送门的坐标和一个int变量（sdoor_coord）表示传送门的个数。
//     然后在输入的for循环中，在判断起点位置的下方同时判断是否是传送门，若是，则存入door_coord。
...
//     如果是，则将get_jewel的值改变。
//     遍历 get _jewel，将获得宝石的二进制表示（参照3）和获得宝石的总数量统计出来，然后判断是否走了重复的路（用walked）。
//     若不是，则判断得到的宝石数量是否达标，若是，则将enchantment改为false（结界消失）。
//     如果是传送门，则遍历door_coord，将除它本身的传送门存入队列。
//     如果找到终点，则判断enchantment是否为false，若是，则输出，作找到路径的标记并且结束数据；
