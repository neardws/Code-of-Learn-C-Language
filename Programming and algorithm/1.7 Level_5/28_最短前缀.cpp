...
// 注意到这里我们不认为空串是字串, 但是每个非空串是它自身的字串. 
// 我们现在希望能用前缀来缩略的表示单词。
// 例如, "carbohydrate" 通常用"carb"来缩略表示. 
// 现在给你一组单词, 要求你找到唯一标识每个单词的最短前缀
// 在下面的例子中，"carbohydrate" 能被缩略成"carboh", 
// 但是不能被缩略成"carbo" (或其余更短的前缀) 因为已经有一个单词用"carbo"开始
...
int main(){
    ...
    while(cin>>a[n])    
        n++;
    for(int i = 0; i < n; i++){
        int len = a[i].size();      // a[i] 字符串的长度
        for(int j = 0; j <= len; j++){      //到Len 
            string tmp = a[i].substr(0, j); // tmp 为 a[i] 从0 到j 的子串
            int flag = 1;       // 是否和其他字符串一致
            for(int k = 0; k < n; k++){     
                if(i != k && tmp == a[k].substr(0, j)){ // 与a[k]子串一致
                    flag = 0;   // 设置标志位
                    break;
                }
            }
            if(flag || j == len){    // 不与其他子串一样 或 遍历到尾
                cout<<a[i]<<" "<<tmp<<endl;
                break;
            }
        }
