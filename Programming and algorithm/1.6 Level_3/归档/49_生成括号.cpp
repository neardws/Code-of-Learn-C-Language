...
void dfs(int n, string path, vector<string>& res, int open, int close) {    // 
    if (open > n || close > open)   // 左括号数量大于n，或者右括号数量大于左括号数量，不合理
        return;
    if (path.length() == 2 * n) {   // 已经生成了一组括号
        res.push_back(path);        // 添加到结果集中
        return;
    }
    dfs(n, path + "(", res, open + 1, close);   // 左括号
    dfs(n, path + ")", res, open, close + 1);   // 右括号
}
int main(){
    int n;
    cin>>n;
