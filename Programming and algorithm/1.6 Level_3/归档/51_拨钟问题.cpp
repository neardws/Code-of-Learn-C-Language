...
//0,1,2,3,4,5,6,7,8
//A,B,C,D,E,F,G,H,I
int clocks[9] = { 0 };	// 时钟状态数组

int steps[9] = { 0 };	//每种操作对应的操作次数

//利用约束关系求出其余操作所需最小次数
void check_road(int i1, int i2, int i3) {		// i1,i2,i3为前3个操作的次数
	int i4 = (4 - ((i1 + i2 + clocks[0]) % 4)) % 4; 	//根据A的约束关系，求出第四个操作的次数 
	...
	//F,G,H
	if (//满足题目要求，全为12点
		(clocks[5] + i3 + i5 + i6 + i9) % 4 == 0 &&		//F钟的状态
		(clocks[6] + i4 + i7 + i8) % 4 == 0 &&			//G钟的状态
		(clocks[7] + i5 + i7 + i8 + i9) % 4 == 0		//H钟的状态
		) {
		...
		if (sum < ans)//取总的路径数最小的
			memcpy(steps, tmp, sizeof(steps));	//更新steps，memcpy是拷贝数组
	}
}

//i1,i2,i3
void find() {
	for (int i = 0; i < 4; ++i) {			// 第一种操作
		for (int j = 0; j < 4; ++j) {		// 第二种操作
			for (int k = 0; k < 4; ++k) {	// 第三种操作
				check_road(i, j, k);
			}
		}
	}
