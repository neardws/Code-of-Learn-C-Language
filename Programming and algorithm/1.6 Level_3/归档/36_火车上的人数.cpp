...
// 现给出的条件是：共有n个车站，始发站上车的人数为a，
// 最后一站下车的人数是m（全部下车）。试问x站开出时车上的人数是多少？

// 输入
// 一行，包含四个整数a，n，m和x，相邻两个整数之间用单个空格隔开。
...
int main(){
    int a,n,m,x;
    cin>>a>>n>>m>>x;
    if(x<=2){printf("%d",a);return 0;}
    s[1]=a,r[1]=r[2]=a;
    for(int i=0;i<=10;i++){  //枚举第二站上车的人数
        s[2]=i;
        for(int j=3;j<n;j++){    //直到n-1站都满足的规律
            s[j]=s[j-1]+s[j-2];
            r[j]=r[j-1]+s[j-2];
        }
        if(r[n-1]==m){
