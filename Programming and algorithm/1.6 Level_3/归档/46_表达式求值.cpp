...
char calculate(char x, char y, char oper){  // 计算 x oper y
    ...
    //     return 'F';
    return 
        ans?'V':'F';    // 三目运算符，ans 为true 返回 V，否则返回 F
}
...
int main(){
    ...
    while(getline(cin,in)){
        stack<char> Oper,num; //新建2个存储字符的栈，oper保存运算符，num保存运算结果
        queue<char> s;        //s为 队列，s就是前缀表达式
        len=in.length();
        i=len;
        in=" "+in;
        while(i>0){// 从右往左，中缀表达式转前缀表达式
            ...
            else if(isalpha(in[i]))    // 如果是字母，则直接进入队列
        ...
        while(!s.empty()){  //计算前缀表达式
            char ch=s.front();      // s.front() 返回队列头元素
            s.pop();    //出队
            if(isalpha(ch))     //如果是字母，则直接进入结果栈
                num.push(ch);
            else 
                Oper.push(ch);  //运算符入栈
